...
function scoreQA(questionTokens, qa) {
    if (questionTokens.length === 0)
        return 0;
    const qaTokens = tokenize(`${qa.question} ${qa.answer}`);
    if (qaTokens.length === 0)
        return 0;
...
async function getOrderSummary(orderIds) {
    const summaries = [];
...
function buildRefusal(question) {
    const polite = [
...
async function useLocalMatcher(question) {
