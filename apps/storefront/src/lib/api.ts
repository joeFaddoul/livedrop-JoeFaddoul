...
function mapProduct(document: z.infer<typeof productSchema>): Product {
  const fallbackId =
...
async function hydrateCatalog(): Promise<Product[]> {
  if (!cachedProducts) {
    if (isLiveBackend) {
      try {
        const payload = (await fetchJson('/api/products')) as { products?: unknown } | unknown[];
    ...
    if (!cachedProducts) {
      cachedProducts = catalogFallback.map((item) =>
        mapProduct(
          productSchema.parse({
            id: item.id,
            title: item.title,
  ...
  cachedProducts = cachedProducts ?? catalogFallback.map((item) =>
    mapProduct(
      productSchema.parse({
        id: item.id,
        title: item.title,
...
export async function getRelatedProducts(id: string, take = 3): Promise<Product[]> {
  if (isLiveBackend) {
    try {
      const payload = (await fetchJson(`/api/products/${id}/related`)) as { related?: unknown[] };
...
export async function lookupCustomer(email: string): Promise<CustomerLookupResponse> {
  if (!isLiveBackend) {
...
export async function placeOrder(
  ...
  try {
    ...
    throw new Error(
      'Your cart contains items we could not match to the catalog. Please refresh the page, rebuild your cart, and try again.',
    );
  }
  ...
  if (invalidProductIds.length > 0) {
    throw new Error(
      'One or more cart items no longer match the live inventory. Please clear your cart and add products again from the catalog.',
    );
  }
...
function normalizeOrderStatus(raw: unknown): OrderStatusRecord | undefined {
  ...
  return {
    ...
    history: data.history?.map((entry) => ({
      status: entry.status as OrderStatusRecord['status'],
...
export function parseOrderEvent(event: MessageEvent<string>): OrderStatusEvent {
  try {
...
export async function fetchDailyRevenueTrend(params?: {
  ...
  if (payload && Array.isArray(payload.points)) {
    return payload.points as DailyRevenuePoint[];
  }
  return [];
}
